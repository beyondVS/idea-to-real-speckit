...
1. **지능형 인쿼리 엔진 (5-Why)**: 사용자의 답변을 실시간으로 분석하여 문제의 본질(Root Cause)을 파악하고, 논리적 깊이가 부족할 경우 지능적인 추가 질의를 생성합니다.
2. **질의 제어 및 연장 (Inquiry Control)**: 기본 5회의 질의 제한을 통해 무한 루프를 방지하며, 필요 시 사용자의 명시적 승인을 얻어 분석을 연장합니다.
3. **분석 확신도 및 상세화 판단**: 도출된 원인에 대한 확신도(매우 높음/보통 등)를 제공하고, 추가적인 상세 분석이 가능한지 논리적으로 판단합니다.
4. **구조화된 보고서 생성**: 분석된 인과관계를 바탕으로 표준화된 Markdown 문제 기술서를 자동 생성합니다.
